...
def get_info(url_list,headers):
    ...
    item_dict['totleprice'] = lis4.find('div',attrs={'class':'price'}).find('span',attrs={'class':'total'}).getText()
    item_dict['meanprice'] = lis4.find('div',attrs={'class':'price'}).find('div',attrs={'class':'unitPrice'}).getText()
    ...
    item_dict['xiaoquname'] = lis4.find('div',attrs={'class':'aroundInfo'}).find('div',attrs={'class':'communityName'}).getText().strip('小区名称').strip('地图')
    item_dict['suozaiarea'] = lis4.find('div',attrs={'class':'aroundInfo'}).find('div',attrs={'class':'areaName'}).getText().strip('所在区域').strip().replace('\xa0','-')
    item_dict['kanfangdata'] = lis4.find('div',attrs={'class':'aroundInfo'}).find('div',attrs={'class':'visitTime'}).getText().strip('看房时间').strip()
    item_dict['lianjiabianhao'] = lis4.find('div',attrs={'class':'aroundInfo'}).find('div',attrs={'class':'houseRecord'}).getText().strip('链家编号').strip('举报')
    # 房源特色部分
...
def main(): 
    ...
    item1.to_csv('C:/Users/Think/Desktop/gulou.csv',encoding='utf8')
    print('成功保存gulou地区数据')           
