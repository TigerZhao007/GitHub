...
#读取停词表。
stopwords[1:10]                                 #查看停词表。
a <- read.table("F:\\rspace\\history\\ciku\\tingcibiao\\Ctingci02.txt",stringsAsFactors=F)
str(a)
...
canlian.paste.se.rm    <- lapply(canlian.paste.se, removeStopWords,stopwords=NA)
...
wordcloud2(canlian.paste.se.ta.so[1:100],shape = 'square')
