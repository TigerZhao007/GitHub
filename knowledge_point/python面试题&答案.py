...
# 求交集
jj1 = [i for i in a if i in b]              # [3, 4]
jj2 = list(set(a).intersection(set(b)))     # [3, 4]

# 求并集
bj1 = list(set(a).union(set(b)))            # [1, 2, 3, 4, 5, 6]
bj2 = list(set(a + b))                      # [1, 2, 3, 4, 5, 6]

# 求差集
cj1 = list(set(b).difference(set(a)))       # [1, 2]
cj2 = list(set(a).difference(set(b)))       # [1, 2]

# 102、生成0-100的随机数~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# random.random()生成0-1之间的随机小数，所以乘以100
import random
res1 = 100 * random.random()             # 17.862691800232046   随机小数
res2 = random.choice(range(0, 100))      # 42  随机整数
res3 = random.randint(1, 100)            # 80  随机整数

# 103、lambda匿名函数好处~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 精简代码，lambda省去了定义函数，map省去了写for循环过程
a = ['苏州', '中国', '哈哈', '', '日本', '', ' ', '', '德国']
res = list(map(lambda x: "填充值" if x == '' or x == ' ' else x, a))

# 104、常见的网络传输协议~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# UDP、TCP、FTP、HTTP、SMTP等等

# 105、单引号、双引号、三引号用法~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 1、单引号和双引号没有什么区别，不过单引号不用按shift，打字稍微快一点。表示字符串的时候，单引号里面可以用双引号，
#    而不用转义字符,反之亦然。
...
# 删除cat、cat2、cat3对象，del方法只有在对象真正被删除时候才能调用打印（也就是cat3被删除时）
del cat                       # 删除cat后，cat2和cat3仍然指向同一地址，说明对象的引用还有2个
print(id(cat2), id(cat3))     # 2564629681432 2564629681432

del cat2                      # 继续删除cat2后，cat3仍然引用对象地址，引用计数还有1个
print(id(cat3))               # 2564629681432

del cat3                      # 删除cat3后，cat、cat2、cat3的引用都被删除，调用del方法，所以打印了del方法被调用这句话。
                              # 而上面还有引用的时候，就不会打印这句话。
...
而是浏览器做了些处理，所以长度依据浏览器的不同有所不同；POST请求在HTTP协议中也没有做说明，
一般来说是没有设置限制的，但是实际上浏览器也有默认值。总体来说，少量的数据使用GET，大量的数据使用POST。
...
df = pd.read_excel(path='A.xlsx', sheet_name='Sheet1')
print(df)
...
import re
s = '小明年龄18岁，工资10000'
res = re.search(r'\d', s).group()       # '1',             search()匹配到第一个匹配到的数据。
res = re.findall(r'\d+', s)             # ['18', '10000'], 满足正则，都匹配，不用加group。

res = re.match('小明', s).group()       # 匹配以’小明‘开头的字符串，并匹配出小明
res = re.match(r'\d+', s)               # 试错，不加guoup为none，匹配不到。不加group不对。
