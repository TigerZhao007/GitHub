...
# 方案~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 1、爬取某网站 20 页图片，每页大概 20~30 张图片
# 2、该网站没有反爬措施
# 3、爬虫全速爬取，不设置休眠时间
# 4、依次爬取每页的图片链接，保存至一个列表中（对于单线程、多进程方案），保存至队列（对于多线程方案），这一步使用单线程；
#    然后用 urllib 下载图片，这一步使用单线程、多线程、多进程分别爬取
# 5、依次使用单线程、多线程、多进程爬取相同的资源
...
# 多进程~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 多进程使用的是 multiprocessing 包下的 Pool类，它会根据电脑所拥有的核心自动创建 Pool 类的实例，也可以手动传入参数；
# 在Pool这个类中有map和map_async两种方法，map方法是阻塞的，也就是map方法之后的代码必须等待map方法执行完成才能继续进行，
...
if __name__ == '__main__':
    ...
    print end_time - start_time 
'''
# map_async 方法是异步的，这一整段代码运行到 map_async 方法时，不会等待这个方法完成，而是继续后面的代码逻辑，
# 而 map_async 方法也在背后继续进行着；其中有个问题就是，当后面的代码运行完之后，就要停止，
# 那么map_async方法没有运行完也会被停止，所以上面的代码比map方法多了两行，join方法的功能就是等待map_async函数执行完成，
# 测试结果：
# 爬取图片数量：571张
# 耗时：38.54秒

# 多线程~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 使用多线程需要 threading 包中 Thread 类，以及配合 Queue 类，多线程中，使用 Queue 代替 List ，Queue 有多种，
# FIFO（先进先出）、LIFO（后进先出）、优先级队列，在此方案中用 FIFO 队列，在原代码做出相应改变：
'''
myqueue = Queue()
....
if pic_link.startswith("http"):
...
'''
# 此段代码的作用：判断队列是否为空，不为空则将里面的 url 取出给download_pic下载，下载完成后，调用Queue类的task_done ，
# 告知电脑此次任务完成，结束资源占用。
...
# 多线程以 32.97 秒的时间排名第一
# 多进程中的 map 和 map_async 效率相差不远 ，位列第二
# 单线程速度最慢，位列第三

# 此次爬取试验中，多线程下载图片相对其他方案来说少了 16 张，具体原因还没有查过，影响较小，暂时不做处理。
# 在网上的讨论中，多线程适用于网页请求以及 I/O 读写操作，多进程适用于 CPU 密集型操作，
# 由于作者水平有限，还没有对线程、进程、全局解释器锁（GIL）等知识进行深入了解，下一次，有机会在做深入学习。
