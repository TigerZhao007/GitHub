...
# 实例的声明及创建~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
'''定义好Student类，就可以根据Student类创建出Student的实例，创建实例是通过类名+()实现的。'''

# 创建Student类的对象bart
bart = Student()

bart      #  <__main__.Student at 0x133df229ba8>
Student   # __main__.Student

'''可以看到，变量bart指向的就是一个Student的object，后面的0x133df229ba8是内存地址，每个object的地址都不一样，
而Student本身则是一个类；可以自由地给一个实例变量绑定属性，比如，给实例bart绑定一个name属性：'''
...
# 创建Student类的对象bart、lisa
jack = Student('Bart Simpson', 59)
bob = Student('Lisa Simpson', 87)
bart = Student('Bart Simpson', 59)

jack  # <__main__.Student at 0x133df229940>
bob   # <__main__.Student at 0x133df229908>
...
'''要定义一个方法，除了第一个参数是self外，其他和普通函数一样。要调用一个方法，只需要在实例变量上直接调用，
除了self不用传递，其他参数正常传入'''
